...
def load_preprocessing(dataset):
    bin_info = pickle.load(open(DATA_PATH / f"{dataset}_bin_info.pkl", "rb"))
    selected_features = pickle.load(open(DATA_PATH / f"{dataset}_selected_features.pkl", "rb"))
    return bin_info, selected_features


@st.cache_resource
def load_background(dataset):
    imgs = np.load(DATA_PATH / f"{dataset}_test_images.npy")
    return np.expand_dims(imgs[:50], axis=-1)


def generate_llm_explanation(pred_class, confidence, inputs):

    risk = "Low Credit Risk" if pred_class == 0 else "High Credit Risk"
...
A deep learning credit scoring system predicted:
...
Explain clearly why the borrower received this risk classification.
Focus on financial indicators such as loan amount, mortgage due, debt burden, and credit history.

Write the explanation so a bank officer can easily understand it.
"""
...
dataset = st.selectbox("Select Dataset", ["HMEQ", "AUSTRALIAN"])

model = models[dataset]
bin_info, selected_features = load_preprocessing(dataset)
background = load_background(dataset)
...
if dataset == "HMEQ":
    ...
    inputs["JOB"] = st.selectbox("Job", ["Mgr", "Office", "Other", "ProfExe", "Sales", "Self"])
...
if st.button("Predict Credit Risk"):
    ...
    if pred_class == 0:
        st.success(f"Low Credit Risk (Confidence: {confidence*100:.2f}%)")
    else:
        st.error(f"High Credit Risk (Confidence: {confidence*100:.2f}%)")
    ...
    shap_map = np.squeeze(sv[0])

    if shap_map.ndim == 3:
        shap_map = shap_map[:, :, 0]

    vmax = np.max(np.abs(shap_map)) or 1

    fig, ax = plt.subplots()

    im = ax.imshow(shap_map, cmap="coolwarm", vmin=-vmax, vmax=vmax)
    ...
    with st.spinner("Generating explanation..."):
        explanation = generate_llm_explanation(pred_class, confidence, inputs)
